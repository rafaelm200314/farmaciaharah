...
if (!isset($_SESSION['employee_id'])) { // Check if the user is logged in.
    header('Location: login.php'); // Redirect to login page if not logged in.
    exit; // Terminate the script execution.
}

$employee_name = $_SESSION['employee_name']; // Get the logged-in employee's name from the session.
?>
                    ...
                    <section class="portfolio-block hire-me">
                        <div class="container">
                            <div class="heading" style="text-align: center;">
                                <h2 style="color: black;">Supplier Form</h2>
                            </div>
                            <form class="border rounded border-0 shadow-lg p-3 p-md-5" data-bs-theme="light" id="addSupplyForm" onsubmit="addSupply(event)" method="POST">
                                <div class="mb-3"><label class="form-label" for="subject">Name</label><input class="form-control" type="text" id="supplyName" name="supplyName" required></div>
                                <div class="mb-3"></div>
                                <div class="mb-3"><label class="form-label" for="subject">Contact</label><input class="form-control" type="text" id="supplyContact" name="supplyContact" required></div>
                                <div class="mb-3"><label class="form-label" for="subject">Address</label><input class="form-control" type="text" id="supplyAddress" name="supplyAddress" required></div>
                                <div class="mb-3">
                                    <div class="row">
                                        <div class="col-md-6 col-xl-12 offset-xl-0 button"><button class="btn btn-primary d-block w-100" type="submit">Add Supplier</button></div>
                                    </div>
                                </div>
                            </form>
